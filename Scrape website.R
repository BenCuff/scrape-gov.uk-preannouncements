...
#Function to scrape HTML from the announcements page
scrape <- function(){
  ...
  #Function for cleaning up outputs
  tidy_up <- function(df){
    return(
      df %>% 
        str_replace(., "\n", "") %>% 
  ...
  results <- 0 #set up results value to monitor how many results are returned on each page
  ...
  repeat{
    ...
    page <- read_html(paste0("https://www.gov.uk/search/research-and-statistics?content_store_document_type=upcoming_statistics&order=release-date-oldest&organisations%5B%5D=uk-health-security-agency&page=", 
                            i))
  ...
  #Return completed output from all pages
  return(output)
...
#Save csv output
write.csv(list, paste0("release_calendar_", Sys.Date(), ".csv"), row.names = F)
